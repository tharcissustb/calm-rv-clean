...
def train_one_fold(train_rows, val_rows, test_rows, cfg, output_dir, device):
...
def main():
    ...
    for event in events:
        ...
        print(f"\n[LOEO] test_event={event} | train={len(train_rows)} val={len(val_rows)} test={len(test_rows)}")

        metrics = train_one_fold(train_rows, val_rows, test_rows, cfg, fold_dir, device)
        metrics["n_test"] = len(test_rows)

        all_results[event] = metrics
        print("TEST:", metrics)

    with open(run_dir / "loeo_results.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2)

    filtered = {k: v for k, v in all_results.items() if v["n_test"] >= args.min_event_size}
    macro_f1s = [v["macro_f1"] for v in filtered.values()]

    summary = {
        "run_id": f"{args.exp_name}_seed{cfg['training']['seed']}",
        "num_events_total": len(all_results),
        "num_events_used": len(filtered),
        "min_event_size": args.min_event_size,
        "avg_macro_f1": float(np.mean(macro_f1s)) if macro_f1s else None,
        "worst_event_macro_f1": float(np.min(macro_f1s)) if macro_f1s else None,
    }

    with open(run_dir / "loeo_summary.json", "w", encoding="utf-8") as f:
