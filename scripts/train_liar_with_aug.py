"""
Train LIAR with LIAR-specific augmentation
"""
...
def train_and_evaluate(train_rows, test_rows, args, cfg, device):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            # For LIAR, we still use alignment but it may not help much
            if args.use_align:
...
def main():
    ...
    parser.add_argument("--aug_path", type=str, default=None, help="Path to augmented data")
    ...
    print("="*60)
    print(f"LIAR Experiment with LIAR-specific Augmentation: {args.exp_name}")
    ...
    print(f"  Augmentation: {args.aug_path}")
    print("="*60)
    
    # Load LIAR data
    data_path = ROOT / cfg["dataset"]["path"]
    all_rows = load_jsonl(data_path)
    print(f"Loaded {len(all_rows)} LIAR examples")
    
    # Split data
    random.shuffle(all_rows)
    train_rows = all_rows[:int(0.8 * len(all_rows))]
    test_rows = all_rows[int(0.8 * len(all_rows)):]
    
    # Load LIAR-specific augmentation if provided
    if args.aug_path:
        aug_path = ROOT / args.aug_path
        if aug_path.exists():
            augmented_rows = load_jsonl(aug_path)
            train_rows = train_rows + augmented_rows
            print(f"Added {len(augmented_rows)} LIAR-augmented examples")
            print(f"Total training set: {len(train_rows)}")
    
    # Run training
    results = train_and_evaluate(train_rows, test_rows, args, cfg, device)
    
    print("\n" + "="*60)
    print(f"LIAR RESULTS with {args.aug_path if args.aug_path else 'no augmentation'}")
    print("="*60)
